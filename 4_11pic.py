...
class car_pic():
    ...
    def blackarea(self,picture, judgepix1,judgepix2):
        ...
        bw = cv2.dilate(thresh, rect)  # 闭运算
        bw = cv2.dilate(bw, rect)  # 闭运算
        bw = cv2.dilate(bw, rect)  # 闭运算
        ...
        # label_image[borders] = -1 # 不同标记用不同颜色显示

        segmentation.clear_border(cleared)
        labels = measure.label(cleared, connectivity=1)  # 8连通区域标记

        bnj = color.label2rgb(labels)  # 根据不同的标记显示不同的颜色
        ...
        if (len(black_area) > 4):
            ...
            if (judge1 == 1 and judge2 == 1):
                ...
                img2 = pp[int(black_area[len(black_area) - 2]):480, blackareazhixin - 100:blackareazhixin + 100]
                gray2 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
                gray2 = cv2.blur(gray2, (3, 3))
                gray2 = cv2.Canny(gray2, 50, 150, apertureSize=3)
                g2 = (gray2[:, :] > 10)
                pix2 = np.sum(g2)
                print('  g2  =', np.sum(g2))
                cv2.imshow('gray2', gray2)
                cv2.imwrite('img2.jpg', gray2)
                if pix2 > judgepix2:
                    pix1 = -1
                    judge2 = 1
            ...
            img1 = pp[int(black_area[0]):280, blackareazhixin - 100:blackareazhixin + 100]
            img2 = pp[int(black_area[2]):480, blackareazhixin - 100:blackareazhixin + 100]
            gray1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
            gray2 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
            gray1 = cv2.blur(gray1, (3, 3))
            gray2 = cv2.blur(gray2, (3, 3))
    ...
    def huop(self, area, pix, r, g, b,ppix1=550,ppix2=550):
        """
        pix1 = []
        pix2 = []
        pix3 = []
        ...
        for vline in range(12):
            if  pixt[vline]==0 and b_pixt[vline]==1:
                pixt[vline]=1
...
if __name__ == '__main__':
    ...
    result = tt.huop("A", 1500, 70, 70, 75)
    print(result)
